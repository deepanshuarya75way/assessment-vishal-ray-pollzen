...
export const checkVoteStatus = async (req, res, next) => {
     try {
          ...
          const existingResponse = await ResponseService.checkExistingResponse(pollId, userId, anonymousId);

          return res.status(200).json(
               new ApiResponse(200, { hasVoted: !!existingResponse }, "Status fetched")
          );
     } catch (error) {
          next(error);
     }
